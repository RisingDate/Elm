...
class CustomFusionDataset(Dataset):
    def __init__(self, dataframe, tokenizer, text_col, num_features):
        self.data = dataframe
    ...
    def __getitem__(self, idx):
        ...
        numeric_data = torch.tensor(row[self.num_features].values, dtype=torch.float32)
        target = torch.tensor(row['interaction_cnt'], dtype=torch.float32)
        return input_ids, attention_mask, numeric_data, target
